...
\begin{enumerate}
  ...
  \item ReportLaunchService realiza validaciones síncronas antes de lanzar el job:
    \begin{itemize}
      ...
      \item \textbf{Verificación de disponibilidad de datos}: invoca \texttt{hasData()} del servicio \texttt{ReportDataProbeService} para comprobar rápidamente si habrá datos suficientes para generar el informe. Si no hay datos, devuelve estado \texttt{NO\_DATA}.
  ...
  \item El \textbf{Job} de Spring Batch consta de un único \texttt{Step} que ejecuta \texttt{GenerateReportTasklet}:
    \begin{itemize}
  ...
  \item Al finalizar el Job, se registran métricas de ejecución y tamaño del fichero.
  
  \item El controlador devuelve el informe generado al cliente como stream binario.
\end{enumerate}
