...
\begin{itemize}
  ...
  \item \textbf{Aterrizaje en Jira}: asignación al EPIC “Improvements for ALSEA Project in 2025 Q3 \& Q4”, con tareas tipificadas como Story, Task o Bug, cada una con subtareas y prioridad.
...
\begin{itemize}
  \item \textbf{Gestión de trabajo}: Jira (tablero Kanban; EPICs, historias de usuario, subtareas y prioridades).
  \item \textbf{Control de versiones y hosting}: Git para control de versiones y GitHub como repositorio remoto y plataforma de revisión mediante Pull Requests.
  \item \textbf{Flujo de trabajo Git}: Gitflow como convención de ramas (feature/release/hotfix) para mantener estabilidad en ramas de producción y organizar el desarrollo.
  \item \textbf{Integración continua / CI \& CD}: GitHub Actions para pipelines que automatizan build, tests y despliegues. Las acciones se integran con SonarCloud para chequeos de calidad y con los registries de artefactos.
  \item \textbf{Registro y gestión de artefactos}: Quay y JFrog Artifactory se emplearon como registries para imágenes Docker y artefactos binarios, facilitando la distribución y el versionado de imágenes.
  \item \textbf{Orquestación de despliegue y automatización}: AWX (interfaz web para Ansible) para ejecutar playbooks y automatizar despliegues en entornos de QA e integración.
  ...
  \item \textbf{Logs y búsqueda}: Elasticsearch como servidor on-premise para almacenamiento y consulta de logs e información de instancias.
  ...
  \item \textbf{Calidad de código}: SonarCloud / SonarQube para análisis estático, cobertura y detección de deuda técnica durante los pipelines de CI.
\end{itemize}

En resumen, se utilizó un flujo de CI/CD basado en GitHub Actions que ejecuta compilaciones, pruebas y análisis de calidad (SonarCloud), publica artefactos en registries (Quay/JFrog) y utiliza AWX para ejecutar despliegues reproducibles con Ansible en entornos de integración y QA. El uso de Gitflow como convención de ramas facilitó la gestión de releases y la colaboración mediante Pull Requests.
...
\subsection{Pruebas manuales nosotras (QA)}
